...
function startSightReader() {
    console.log("Sight Reader Started!");
}
    ...
    fetch(`/playlist/${filename}`)
        ...
        .then(data => {
            ...
            data.forEach((item, i) => {
                ...
                listItem.setAttribute('data-playlist-index', i);
                listItem.textContent = item;
                listItem.addEventListener('click', () => goto_playlist_index(i));
                playlist.appendChild(listItem);
            });

            if (playlist_files.length > 0) {
                update_playlist();
            }
        })
        .catch(error => {
...
function load_abc_file(filename) {
    ...
    fetch(`music/${filename}`)
        ...
        .then(data => {
            // Process the ABC file like the server did
            const processedData = process_abc_data(data);
...
function process_abc_data(data) {
    ...
    lines.forEach(line => {
        ...
        // Check for headers and ignore specified ones
        if (line.length >= 2 && line[1] === ':' && /^[A-Za-z]$/.test(line[0])) {
            if (ignoredHeaders.has(line[0].toUpperCase())) {
                return; // Skip ignored headers
            }
            headers.push(line); // Keep relevant headers
        } else {
            notes.push(line); // Keep note-related content
        }
    });

    return [...headers, ...notes].join('\n');
}
